...
def get_tenant_database_alias():
    return getattr(settings, 'TENANT_DB_ALIAS', 'default')
...
class tenant_context(ContextDecorator):
    ...
    def __enter__(self):
        self.connection = connections[self.database or 'default']
        self.previous_tenant = connection.tenant
...
def schema_exists(schema_name, database='default'):
...
def schema_rename(tenant, new_schema_name, database='default', save=True):
    """
