...
class TenantMixin(models.Model):
    ...
    def _drop_schema(self, force_drop=False, db='default'):
        """ Drops the schema"""
        connection = connections[db]
        has_schema = hasattr(connection, 'schema_name')
        ...
        if has_schema and schema_exists(self.schema_name, database=db) and (self.auto_drop_schema or force_drop):
            self.pre_drop()
    ...
    def delete(self, force_drop=False, *args, **kwargs):
        ...
        db = self.get_db(**kwargs)
        if db:
            self._drop_schema(force_drop, db=db)
        else:
            self._drop_schema(force_drop)
        super().delete(*args, **kwargs)

    def create_schema(self, check_if_exists=False, sync_schema=True,
                      verbosity=1, db='default'):
        """
        ...
        if check_if_exists and schema_exists(self.schema_name, database=db):
            return False
        ...
        if sync_schema:
            if fake_migrations:
                ...
                call_command('migrate_schemas',
                             ...
                             verbosity=verbosity,
                             database=db,
                             app_label='commons_pg')
            else:
                # create the schema
                print(f'---Create schema {self.schema_name}---')
...
class DomainMixin(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        db = self.get_db(**kwargs)
        if db:
            domain_list = self.__class__.objects.using(db).filter(tenant=self.tenant, is_primary=True).exclude(pk=self.pk)
