    ...
    def handle(self, *args, **options):
        ...
        if self.sync_tenant:
            if self.schema_name and self.schema_name != self.PUBLIC_SCHEMA_NAME and ',' not in self.schema_name:
                if not schema_exists(self.schema_name, database=self.options.get('database', 'default')):
                    raise RuntimeError('Schema "{}" does not exist'.format(
                        self.schema_name))
                elif has_multi_type_tenants():
                    type_field_name = get_multi_type_database_field_name()
                    tenants = get_tenant_model().objects.objects.using(
                        self.options.get('database', 'default')).only(
                        'schema_name').only('schema_name', type_field_name)\
                ...
                if ',' in self.schema_name:
                    ...
                    if has_multi_type_tenants():
                        type_field_name = get_multi_type_database_field_name()
                        tenants = get_tenant_model().objects.using(
                            self.options.get('database', 'default')).only(
                            'schema_name').only('schema_name', type_field_name)\
